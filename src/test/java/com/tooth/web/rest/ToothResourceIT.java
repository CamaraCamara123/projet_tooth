...
class ToothResourceIT {
    ...
    void createTooth() throws Exception {
        ...
        // Create the Tooth
        ToothDTO toothDTO = toothMapper.toDto(tooth);
        restToothMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(toothDTO)))
    ...
    void putExistingTooth() throws Exception {
        ...
        ToothDTO toothDTO = toothMapper.toDto(updatedTooth);

        restToothMockMvc
            .perform(
                put(ENTITY_API_URL_ID, toothDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(toothDTO))
            )
            .andExpect(status().isOk());
    ...
    void putWithMissingIdPathParamTooth() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restToothMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(toothDTO)))
    ...
    void patchNonExistingTooth() throws Exception {
        ...
        restToothMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, toothDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(toothDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamTooth() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restToothMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(toothDTO)))
            .andExpect(status().isMethodNotAllowed());
